...
public class AltCache<T: DataConvertible where T.Result == T, T : DataRepresentable> : Cache<T> {
    public override init(name: String) {
        super.init(name: name)
    }
    public override func fetch(URL URL : NSURL, formatName: String = HanekeGlobals.Cache.OriginalFormatName,  failure fail : Fetch<T>.Failer? = nil, success succeed : Fetch<T>.Succeeder? = nil) -> Fetch<T> {
